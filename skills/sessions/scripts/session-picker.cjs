...
// ---------------------------------------------------------------------------
// Auto-generated session detection
// ---------------------------------------------------------------------------

/**
 * Check if a session is auto-generated (local-command sessions created by
 * /exit, /clear, etc. — not real conversations worth keeping).
 *
 * Detects sessions whose first user message starts with "<local-command-caveat>"
 * or contains only slash command output like /exit, /clear, /compact.
 *
...
function isAutoGenerated(session) {
  ...
  let raw;
  try {
    raw = fs.readFileSync(session.filePath, 'utf8');
  } catch (_) {
    return false;
  }
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (entry.type === 'user' && entry.message) {
        ...
        // Local-command sessions start with this caveat tag
        if (text.includes('<local-command-caveat>')) return true;
        // Sessions that are just slash commands with no real content
        if (text.includes('<command-name>/exit</command-name>')) return true;
        if (text.includes('<command-name>/clear</command-name>')) return true;
        return false;
      }
    } catch (_) {
      continue;
...
/**
 * Delete a session file and its subdirectory.
...
function handleCleanAuto() {
  ...
  process.stdout.write(JSON.stringify({
    ...
    remaining,
    deleted,
    message: `Cleaned ${deleted.length} auto-generated session(s). ${remaining} remaining.`
  }) + '\n');
  process.exit(0);
}
